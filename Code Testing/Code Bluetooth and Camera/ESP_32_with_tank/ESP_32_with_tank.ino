...
void setup() {
  ...
  Serial.write(50); // 50 is written to serial monitor indicating Wifi Connected
    delay(500);
/*
  ...
  if (Firebase.get(firebaseData, "/ZetaBot/UpButton")) {
    if (firebaseData.dataType() == "string") {
      ...
      if (De2 == "8") {
        ...
        digitalWrite(gpRf, LOW); //Forward
        digitalWrite(gpRb, LOW); //Forward
        digitalWrite(gpLf, LOW); //Forward
        digitalWrite(gpLb, LOW); //Forward
        }
     }
    }
